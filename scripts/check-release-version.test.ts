...
describe('release version validation', () => {
  it('accepts the tag matching every package version', () => {
    expect(validateReleaseVersions('v0.2.1')).toBe('0.2.1')
  })

  it('rejects a tag that does not match repository versions', () => {
    expect(() => validateReleaseVersions('v0.2.0')).toThrow('does not match release tag')
  })

  it('rejects malformed tags', () => {
    expect(() => validateReleaseVersions('release-0.2.1')).toThrow('form vX.Y.Z')
